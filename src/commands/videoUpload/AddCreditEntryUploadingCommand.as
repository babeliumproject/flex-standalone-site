package commands.videoUpload
{
	...
	public class AddCreditEntryUploadingCommand implements ICommand, IResponder
	{
		public function execute(event:CairngormEvent):void
		{
			
			DataModel.getInstance().historicData.userId = DataModel.getInstance().loggedUser.id;
			DataModel.getInstance().historicData.changeAmount = DataModel.getInstance().prefDic['uploadExerciseCredits'];
			DataModel.getInstance().historicData.changeDate = new Date().toString();
			DataModel.getInstance().historicData.changeType = "exercise_upload";
			DataModel.getInstance().historicData.videoEvaluationId = 0;
			DataModel.getInstance().historicData.videoResponseId = 0;
		...
		public function fault(info:Object):void
		{
			...
			Alert.show("Error Adding Credit History Entry: "+faultEvent.message);
