...
class CleanupTask{
     ...
     * @param int $mhours
     *      How many hours since the last modification time of the file to consider it eligible to be deleted 
     */
    private function _deleteFiles($pathToInspect, $referencedResources, $mhours=2){
        if($referencedResources){
            ...
            while (false !== ($entry = $folder->read())) {
                ...
                if(!is_dir($entryFullPath)){
                    ...
                    //Exclude the merged response file (if available)
                    $entryMerged = str_replace('_merge','',$entry);
                    if( $entryInfo['extension'] == 'flv' && !in_array($entryMerged, $referencedResources) ){

                        //Check modification time of the file
                        //If it was modified $mhours ago and is not referenced in the database it is very likely the user isn't watching it and won't watch it anymore
                        if( ($mtime = filemtime ($entryFullPath)) && ((time()-$mtime)/3600 > $mhours) ){
